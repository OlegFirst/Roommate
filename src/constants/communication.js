...
(async () => {
	...
	const failedSignInResponse = await fetch(
 ...
 */
	...
	const deleteListingResponse = await fetch(
		'http://localhost:8080/listing/delete',
		{
			method: 'POST',
			headers: {
	...
	//
	//
	const findListingResponse = await fetch(
		'http://localhost:8080/listing/find?priceMax=3000',
		{
			method: 'GET',
			headers: { 'content-type': 'application/json' },
		}
	...
	const findListingResponse2 = await fetch(
		'http://localhost:8080/listing/find',
		{
			method: 'POST',
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify({
